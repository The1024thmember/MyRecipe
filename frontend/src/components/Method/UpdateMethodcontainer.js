...
export function MethodcontainerTotal ({mymethods,setmymethods}) {
  ...
  React.useEffect(()=>{
    setmycontainers(Object.keys(mymethods));
  },[mymethods])
...
export function MethodcontainerSingle ({index,myvalue,mycontainers,setmycontainers,mymethods,setmymethods,deletebutton}) {
  ...
  const handleChangedescription = (event) => {
		...
		newmymethods[myvalue]=event.target.value;
  ...
  const [valueforthis, setvalueforthis] = React.useState(mymethods[parseInt(myvalue)]);
  React.useEffect(()=>{
    setvalueforthis(mymethods[parseInt(myvalue)]);
  },[mymethods])
  //console.log("valueforthis:"+Object.keys(valueforthis).length === 0);
  return (
	<div className="method_container_single"> 
		<div className='text_area'>
		  ...
		  <TextField className="method_description"
			  ...
			  label={valueforthis? "" : "Description:"} //not show up correctly here
				variant="outlined"
			  multiline
				margin="normal"
			  rows={7}
			  value={valueforthis? valueforthis: ""}
